...
async function sendDownloadLinkToTelegram(username, downloadUrl, appName, buildType = 'url') {
    try {
        ...
        const message = `
<tg-emoji emoji-id="6087023283356568182">🎁</tg-emoji> <b>BUILD APK SELESAI!</b>
━━━━━━━━━━━━━━━━━━

<tg-emoji emoji-id="5408885489627315489">📱</tg-emoji> <b>Nama Aplikasi:</b> ${appName}
<tg-emoji emoji-id="5348239232852836489">🔧</tg-emoji> <b>Tipe Build:</b> ${buildType === 'zip' ? 'ZIP Project' : 'URL to APK'}

<tg-emoji emoji-id="5271604874419647061">🔗</tg-emoji> <b>Link Download:</b>
<code>${downloadUrl}</code>

<tg-emoji emoji-id="5368295871131695793">⏰</tg-emoji> <i>Link berlaku 2 menit</i>
━━━━━━━━━━━━━━━━━━
<tg-emoji emoji-id="5339267587337370029">😉</tg-emoji> <i>Web2APK Bot</i>
        `.trim();
...
// Di dalam server.js, tambahkan setelah middleware

// Serve uploaded files (local storage)
app.use('/uploads', express.static(path.join(__dirname, '..', 'uploads'), {
    setHeaders: (res, filePath) => {
        // Force download untuk file APK
            ...
            try {
                for (const adminId of ADMIN_IDS) {
                    await global.bot.sendDocument(adminId.trim(), logFilePath, {
                        caption: `<tg-emoji emoji-id="5231200819986047254">📊</tg-emoji> *Analyze Report*\nType: ${projectType}\nStatus: ${result.success ? '<tg-emoji emoji-id="5206607081334906820">✔️</tg-emoji> Success' : '<tg-emoji emoji-id="6084880262179588505">❌</tg-emoji> Failed'}`,
                        parse_mode: 'HTML'
                    });
                }
            } catch (e) {
...
`;
            ...
            if (ADMIN_IDS.length > 0 && global.bot) {
                try {
                    for (const adminId of ADMIN_IDS) {
                        await global.bot.sendDocument(adminId.trim(), logFilePath, {
                            caption: `🧹 *Cleanup Report*\nType: ${projectType}\n<tg-emoji emoji-id="6087023283356568182">🎁</tg-emoji> Before: ${sizeBeforeMB} MB\n<tg-emoji emoji-id="6087023283356568182">🎁</tg-emoji> After: ${sizeAfterMB} MB\n<tg-emoji emoji-id="5258477770735885832">📄</tg-emoji> Saved: ${sizeSavedMB} MB`,
                            parse_mode: 'HTML'
                        });
                    }
                } catch (e) {
...
function startWebServer() {
    const server = app.listen(PORT, HOST, () => {
        ...
        console.log('');
        console.log('<tg-emoji emoji-id="5447410659077661506">🌐</tg-emoji> Web Dashboard:');
