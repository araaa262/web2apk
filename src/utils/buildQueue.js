...
class BuildQueue {
    constructor() {
        ...
        console.log(`<tg-emoji emoji-id="5348239232852836489">🔧</tg-emoji> Build Queue: max ${this.maxConcurrent} concurrent build(s)`);
        console.log(`<tg-emoji emoji-id="5231200819986047254">📊</tg-emoji> Stats: ${this.stats.total} total, ${this.stats.success} success, ${this.stats.failed} failed`);
        console.log(`<tg-emoji emoji-id="5229011542011299168">👑</tg-emoji> Admin IDs: ${this.adminIds.length} configured`);
    ...
    load() {
        try {
            if (fs.existsSync(this.queueFile)) {
                ...
                console.log(`<tg-emoji emoji-id="5818955300463447293">🗂</tg-emoji> Loaded queue: ${this.queue.length} pending builds`);
            }
        } catch (error) {
    ...
    addToQueue(chatId, buildData, type = 'url', userName = 'User') {
        ...
        if (isPriority) {
            ...
            console.log(`[Queue] <tg-emoji emoji-id="5433758796289685818">👑</tg-emoji> Priority build queued: ${chatId} at position ${insertIndex + 1}`);
        } else {
            this.queue.push(queueItem);
            console.log(`[Queue] <tg-emoji emoji-id="5346077597287589711">📝</tg-emoji> Build queued: ${chatId} at position ${this.queue.length}`);
        }
        ...
        return {
            ...
            isPriority,
            message: isPriority
                ? `<tg-emoji emoji-id="5433758796289685818">👑</tg-emoji> Prioritas! Posisi: #${position}`
                : `<tg-emoji emoji-id="5346077597287589711">📝</tg-emoji> Antrian #${position}, ~${estimatedWait} menit`
        };
    }
    ...
    removeFromQueue(chatId) {
        ...
        if (index >= 0) {
            ...
            this.save();
            console.log(`[Queue] <tg-emoji emoji-id="5258130763148172425">🗑</tg-emoji> Removed from queue: ${chatId}`);
    ...
    acquire(chatId, buildData = null, type = 'url', userName = 'User') {
        if (this.activeBuilds.has(chatId)) {
            console.log(`[Queue] <tg-emoji emoji-id="5420323339723881652">⚠️</tg-emoji> Chat ${chatId} already building`);
            return false;
        }

        if (this.activeBuilds.size >= this.maxConcurrent) {
            console.log(`[Queue] <tg-emoji emoji-id="5240241223632954241">🚫</tg-emoji> Full (${this.activeBuilds.size}/${this.maxConcurrent})`);
            return false;
        }
        ...
        console.log(`[Queue] <tg-emoji emoji-id="5206607081334906820">✔️</tg-emoji> Started: ${chatId} by ${userName} (${this.activeBuilds.size}/${this.maxConcurrent})`);
        return true;
    }
    ...
    release(chatId, success = true) {
        ...
        console.log(`[Queue] ${success ? '<tg-emoji emoji-id="5206607081334906820">✔️</tg-emoji>' : '<tg-emoji emoji-id="6084880262179588505">❌</tg-emoji>'} Completed: ${chatId} (${durationSec}s)`);
    ...
    async processNext() {
        if (this.queue.length === 0) {
            console.log(`[Queue] <tg-emoji emoji-id="5472107610087889157">📭</tg-emoji> No pending builds`);
            return;
        }

        if (this.isBusy()) {
            console.log(`[Queue] <tg-emoji emoji-id="5215327832040811010">⏳</tg-emoji> Still busy (${this.activeBuilds.size}/${this.maxConcurrent})`);
            return;
        }
        ...
        console.log(`[Queue] <tg-emoji emoji-id="5445284980978621387">🚀</tg-emoji> Auto-starting: ${nextBuild.chatId} (${nextBuild.userName})`);
        ...
        if (!acquired) {
            ...
            console.log(`[Queue] <tg-emoji emoji-id="5420323339723881652">⚠️</tg-emoji> Failed to acquire slot for ${nextBuild.chatId}, returning to queue`);
        ...
        if (this.onBuildStart) {
            try {
                ...
                console.error(`[Queue] <tg-emoji emoji-id="6084880262179588505">❌</tg-emoji> Auto-start error for ${nextBuild.chatId}:`, error.message);
                // Release the slot so queue can continue
                this.release(nextBuild.chatId, false);
            }
        } else {
            console.error(`[Queue] <tg-emoji emoji-id="6084880262179588505">❌</tg-emoji> onBuildStart callback not set! Releasing slot.`);
            this.release(nextBuild.chatId, false);
        }
    }
    ...
    getQueueStatusMessage(chatId) {
        ...
        if (info.processing >= this.maxConcurrent) {
            ...
            statusIcon = '<tg-emoji emoji-id="5215685881989442149">🟢</tg-emoji>';
            statusText = 'Aktif';
        }

        let message = `<tg-emoji emoji-id="5346077597287589711">📝</tg-emoji> <b>Status Antrian</b>\n━━━━━━━━━━━━━━━━━━\n\n`;
        message += `${statusIcon} <b>Server:</b> ${statusText}\n`;
        message += `<tg-emoji emoji-id="5231200819986047254">📊</tg-emoji> <b>Slot:</b> ${info.processing}/${info.maxConcurrent} terpakai\n`;
        ...
        message += `✅ ${stats.success} berhasil | <tg-emoji emoji-id="6084880262179588505">❌</tg-emoji> ${stats.failed} gagal\n`;
        message += `<tg-emoji emoji-id="5368295871131695793">⏰</tg-emoji> Rata-rata: ${stats.avgTime}s\n`;

        if (isActive) {
            const build = this.activeBuilds.get(chatId);
            const duration = Math.round((Date.now() - build.startTime) / 1000);
            message += `\n<tg-emoji emoji-id="5017470156276761427">🔄</tg-emoji> <b>Build Anda sedang berjalan</b> (${Math.floor(duration / 60)}m ${duration % 60}s)`;
        } else if (position > 0) {
            const estimatedWait = this.getEstimatedWait(position);
            message += `\n<tg-emoji emoji-id="6084443648689179160">😎</tg-emoji> <b>Posisi Anda:</b> #${position} dari ${info.waiting}`;
            message += `\n<tg-emoji emoji-id="5368295871131695793">⏰</tg-emoji> <b>Estimasi:</b> ~${estimatedWait} menit`;
        }
    ...
    getAdminQueueMessage() {
        ...
        let message = `<tg-emoji emoji-id="5433758796289685818">👑</tg-emoji> <b>Admin - Queue Manager</b>\n━━━━━━━━━━━━━━━━━━\n\n`;

        // Active builds
        message += `<b><tg-emoji emoji-id="5213452215527677338">⏳</tg-emoji> Sedang Berjalan (${activeBuilds.length}/${this.maxConcurrent}):</b>\n`;
        ...
        // Queue
        message += `\n<b><tg-emoji emoji-id="5346077597287589711">📝</tg-emoji> Antrian (${queueList.length}):</b>\n`;
        if (queueList.length === 0) {
            message += `<i>Kosong</i>\n`;
        } else {
            for (const item of queueList.slice(0, 10)) {
                const priority = item.priority ? '<tg-emoji emoji-id="5433758796289685818">👑</tg-emoji> ' : '';
    ...
    forceRelease(chatId = null) {
        if (chatId) {
            if (this.activeBuilds.has(chatId)) {
                console.log(`[Queue] <tg-emoji emoji-id="5017470156276761427">🔄</tg-emoji> Force release: ${chatId}`);
                this.activeBuilds.delete(chatId);
                this.processNext();
            }
        } else {
            console.log(`[Queue] <tg-emoji emoji-id="5017470156276761427">🔄</tg-emoji> Force release ALL`);
